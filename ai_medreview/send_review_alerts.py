...
def find_negative_reviews(df):
    ...
    """
    if not all(
        col in df.columns for col in SENTIMENT_COLUMNS + SCORE_COLUMNS + [TIME_COLUMN]
    ):
        missing_cols = [
            col
            for col in SENTIMENT_COLUMNS + SCORE_COLUMNS + [TIME_COLUMN]
    ...
    try:
        ...
        if len(df) < initial_rows:
            logger.warning(
                f"Dropped {initial_rows - len(df)} rows due to invalid timestamps in '{TIME_COLUMN}'."
            )

        # Check if the DataFrame is empty after dropping NaNs
        if df.empty:
            logger.warning(
                "DataFrame is empty after dropping rows with invalid timestamps."
            )
            return pd.DataFrame()

    except Exception as e:
        logger.error(
            f"Error processing '{TIME_COLUMN}' column for datetime conversion: {e}"
        )
        return pd.DataFrame()
    ...
    recent_reviews_df = df[
        df[TIME_COLUMN] >= time_threshold
    ...
    negative_filtered_df = recent_reviews_df[
        (
            (
                recent_reviews_df[SENTIMENT_COLUMNS[0]].astype(str).str.lower()
                == "negative"
            )
        ...
        | (
            (
                recent_reviews_df[SENTIMENT_COLUMNS[1]].astype(str).str.lower()
                == "negative"
            )
...
if __name__ == "__main__":
    try:
        ...
        logger.info(
            f"Found {len(negative_reviews)} negative reviews in the last {TIMEFRAME_HOURS} hours."
        )
        ...
        if not negative_reviews.empty:
            ...
            if "surgery" in negative_reviews.columns:
                ...
                for surgery_name, surgery_df in grouped_reviews:
                    if TEST_MODE:
                        logger.info(
                            f"TEST_MODE is ON. Sending email for {surgery_name} to test recipient: {TEST_RECIPIENT_EMAIL}"
                        )
                        subject, text_body, html_body = format_email_content(
                            surgery_name, surgery_df, TIMEFRAME_HOURS
                        )
                        send_alert_email(
                            TEST_RECIPIENT_EMAIL, subject, text_body, html_body
                        )
                    else:
                        ...
                        if to_email:
                            logger.info(
                                f"Processing negative reviews for surgery: {surgery_name}"
                            )
                            subject, text_body, html_body = format_email_content(
                                surgery_name, surgery_df, TIMEFRAME_HOURS
                            )
                            ...
                            logger.warning(
                                f"Email address not found for surgery: {surgery_name} (looked for secret '{email_secret_name}')"
                            )
            else:
                logger.error(
                    "'surgery' column not found in the filtered negative reviews DataFrame."
                )
