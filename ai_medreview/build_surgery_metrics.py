    ...
    try:
        ...
        for col in date_columns:
            ...
            if nat_count > 0 and nat_count > 0.1 * len(df):  # More than 10% NaT
                print(
                    f"Warning: Column '{col}' has {nat_count} ({nat_count/len(df):.1%}) NaT values after conversion"
                )
...
def pre_process_dfs(dfs_dict):
    ...
    try:
        ...
        if "appointments" not in dfs_dict or "surgery" not in dfs_dict:
            raise KeyError(
                "Missing required DataFrames: 'appointments' and/or 'surgery'"
            )
        ...
        clean_surgery_list = [
            surgery_name.replace("-", " ") for surgery_name in surgery_list
        ]

        # Create regex pattern to match any surgery name followed by optional space
        pattern = "|".join(
            re.escape(surgery) + r"\s?" for surgery in clean_surgery_list
        )

        # Remove surgery names from rota_type
        dfs_dict["appointments"]["rota_type"] = dfs_dict["appointments"][
            "rota_type"
        ...
        dfs_dict["appointments"].loc[
            dfs_dict["appointments"]["appointment_status"].isin(
                ["Arrived", "In Progress"]
            ),
            "appointment_status",
        ] = "Finished"
        ...
        # -----------------------------
        dfs_dict["appointments"].rename(
            columns={"appointment_date": "event_date"}, inplace=True
        )
...
def monthly_cxs_demand(df, surgery_id=2, verbose=False):
    ...
    under_50 = len(
        surgery_female_pts[surgery_female_pts["age"] < 50]
    )  # Fixed: changed <= 50 to < 50
    ...
    if verbose:
        print(
            f"Surgery {surgery_id} - Yearly Cx Screening demand: {round(woman_cx_per_year)}"
        )
...
def one_surgery_df_dict(df_dict, surgery_id=2):
    ...
    if (
        "surgery" in df_dict
        and "surgery_id" in df_dict["surgery"].columns
        and surgery_id not in df_dict["surgery"]["surgery_id"].values
    ):
    ...
    for ts_df in ts_list:
        ...
        try:
            ...
            if "surgery_id" not in df_dict[ts_df].columns:
                print(
                    f"Warning: 'surgery_id' column not found in {ts_df} DataFrame. Using all data."
                )
                ...
                if filtered_df.empty:
                    print(
                        f"Warning: No data found for surgery_id {surgery_id} in {ts_df} DataFrame."
                    )
            ...
            if ts_df == "appointments" and not filtered_df.empty:
                if "appointment_status" in filtered_df.columns:
                    ...
                    mask_used = (filtered_df["appointment_status"] == "Finished") | (
                    ...
                    surgery_df_dict[ts_df + "_dna"] = filtered_df[
                        filtered_df["appointment_status"] == "Did Not Attend"
                    ]
                else:
                    print(
                        f"Warning: 'appointment_status' column not found in {ts_df} DataFrame."
                    )
...
def make_ts_df_dict(df_dict, surgery_id=2):
    ...
    for df_name in screening_list:
        try:
            ...
            if surgery_df_dict[df_name].empty:
                print(
                    f"Warning: {df_name} DataFrame is empty. Creating empty result DataFrame."
                )
            ...
            if "event_date" not in surgery_df_dict[df_name].columns:
                print(
                    f"Warning: 'event_date' column missing in {df_name} DataFrame. Creating empty result DataFrame."
                )
                surgery_df_dict[df_name] = pd.DataFrame(columns=["count"])
                continue
            ...
            surgery_df_dict[df_name] = (
                surgery_df_dict[df_name].set_index("event_date").resample("ME").count()
            )

            # Drop columns if there are any to drop
            if cols_to_drop:
                surgery_df_dict[df_name] = surgery_df_dict[df_name].drop(
                    columns=cols_to_drop
                )

            # If DataFrame is not empty, set the first column as 'count'
            if (
                not surgery_df_dict[df_name].empty
                and len(surgery_df_dict[df_name].columns) > 0
            ):
                surgery_df_dict[df_name] = (
                    surgery_df_dict[df_name].iloc[:, 0].to_frame()
                )
    ...
    for df_name in appointment_list:
        try:
            ...
            cols_to_drop = [
                col
                for col in ["surgery_id", "pt_id", "rota_type", "appointment_status"]
                if col in surgery_df_dict[df_name].columns
            ]
...
def make_ts_dataframe(dfs_dict, surgery_id=2):
    ...
    try:
        if "surgery" in dfs_dict and "list_size" in dfs_dict["surgery"].columns:
            ...
            if not surgery_row.empty and "list_size" in surgery_row.columns:
                ...
                print(
                    f"Using dynamic list_size: {list_size} for surgery_id: {surgery_id}"
                )
            else:
                # Fallback to default if surgery_id not found
                list_size = 4300
                print(
                    f"Surgery ID {surgery_id} not found in surgery DataFrame. Using default list_size: {list_size}"
                )
        else:
            # Fallback to default if surgery DataFrame or list_size column not available
            list_size = 4300
            print(
                f"Surgery DataFrame or list_size column not available. Using default list_size: {list_size}"
            )
    except Exception as e:
    ...
    try:
        ...
        if (
            not ts_dict["cx_screening"].empty
            and "count" in ts_dict["cx_screening"].columns
        ):
        ...
        if bowel_dfs_valid:
            bowel_total = (
                ts_dict["bowel_normal"]
                + ts_dict["bowel_non_responder"]
                + ts_dict["bowel_positive"]
            )
            ...
            ts_dict["bowel_screening_rate"] = pd.DataFrame(
                columns=["bowel_screening_rate"]
            )
            ...
            try:
                ...
                if not ts_dict[metric].empty and "count" in ts_dict[metric].columns:
                    ...
                    ts_dict[metric] = pd.DataFrame(
                        normalized_count, index=ts_dict[metric].index
                    ).fillna(0.0)
    ...
    try:
        ...
        for metric in metrics_to_combine:
            if metric not in ts_dict:
                print(
                    f"Warning: {metric} missing from ts_dict. Creating empty DataFrame."
                )
                ts_dict[metric] = pd.DataFrame(columns=[metric])
...
def load_data():
    ...
    if missing_critical:
        raise RuntimeError(
            f"Failed to load critical tables: {', '.join(missing_critical)}"
        )
...
def plot_heatmap(
    combined_df, surgery_id=2, figsize=(12, 8), cmap="YlOrRd", annot=True, title=None
):
    """
    ...
    # Format axis labels
    ax.set_xlabel(
        "Month (MMM-YY)", fontsize=12
    )  # Updated to reflect abbreviated month format
...
if __name__ == "__main__":
    ...
    save_heatmap(
        combined_df,
        "reports/surgery_metrics_heatmap.png",
        figsize=(14, 10),
        cmap="YlOrBr",
        annot=True,
